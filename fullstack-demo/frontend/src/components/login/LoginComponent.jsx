...
const LoginComponent = () => {
    ...
    return (<div className="flex flex-col items-center">
        <form className="flex flex-col gap-3 bg-gray-600 p-4 rounded-xl text-white" onSubmit={handleSubmit}>
            ...
            <GreenButton text={loginMode ? "Login" : "Signup"} disabled={isLoading || !(loginMode ? LOGIN_FIELDS : SIGNUP_FIELDS).every(field => (field.required ? formData[field.property]?.trim() : true))} type="submit" />
        </form>
    </div>)
}
